...
This tutorial will give you a good idea of how outline styles work and what they can do. If you decide to create your own style you should also closely examine the default outline style that's [include](https://github.com/jessegrosjean/bike-extension-kit/tree/main/src/!bike.bkext/style) in the Bike extension kit.&#x20;

Each outline style is ordered list of rules. Each rule is composed of an [outline path](../../using-bike-2/using-outline-paths.md) and a callback function. For each outline element that is styled the following steps are performed:

1. Default style object is created.
2. Creates a list of the rules that match the element.
...
We will start by creating a empty outline style in `style/main.ts`:
...
Replace `style/main.ts` to see structure more clearly:
...
While these rules are simple you can also add decorations to text runs, just like you can add them to rows and row's text. Try creating a rule to show text with the `@highlight` attribute. You can add/remove that attribute from text by using Format > Highlight.
...
With that rule in place when you create a task in your outline it will have a red background. Decorations can be more then just colors, they can also show images using the contents property.
...
Creating a full style that supports all of Bike's features quite complex. Here are some ways to offload that work to existing outline styles, such as the default style that ships with Bike:
...
* Read through the API's document in the [Bike Extension Kit](https://github.com/jessegrosjean/bike-extension-kit)
