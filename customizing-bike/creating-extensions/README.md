...
Use extensions to customize and enhance the functionality of Bike.

Extensions can add new commands, views, and other features to Bike. Sensitive features, such as network requests and clipboard access, are protected by a permission system.
...
The kit requires some setup. The first step to Bike extension development is to download the extension kit and follow the kit's README.md setup instructions. Once you've got it working, the development cycle is fast–save change to extension, see results immediately in Bike.

## Extension Development Overview

You've setup the kit and built & installed some existing extensions. Now we'll take a closer look at what an individual extension looks like and what it can do.
...
Each extension folder has a `manifest.json` which contains name, permissions, and other metadata. Properties are documented in schemas/manifest.schema.json.

Each subfolder corresponds to a different context where the extension code can run. These contexts are run separately and have different availible API. An extension might not need to use all contexts. Delete the subfolder for each unused context.
...
The style context independent, and only needed by extensions that provide outline styles.

## Create Your First Extension

You should have the Bike Extension Kit installed and open in Visual Studio Code.
...
### Rebuild & Instal Extension (Watch Mode)
...
A background process monitors your extension sources for changes. Sometimes it may need a restart but generally enhances development speed. The rest of the tutorials assume you are in watch mode.
